...
module.exports = {
  ...
  execute: async (message, args, client, prefix) => {
    ...
    try {
      ...
      if (!player)
        return message.channel.send({ embeds: [new MessageEmbed().setColor(client.embedColor).setTimestamp().setDescription("Hk có bật bài nào hết")] });
      if (res.loadType === 'LOAD_FAILED') {
        if (!player.queue.current) player.destroy();
        throw res.exception;
      }
    } catch (err) {
      return message.reply(`lỗi trong lúc kiếm bài cho m gùi: ${err.message}`);
    }
    switch (res.loadType) {
      case 'NO_MATCHES':
        if (!player.queue.current) player.destroy();
        return message.channel.send({ embeds: [new MessageEmbed().setColor(client.embedColor).setTimestamp().setDescription(`Hk có thấy bài nào tên - ${search}`)]});
      case 'TRACK_LOADED':
        ...
        if (!player.playing && !player.paused && !player.queue.size) {
          ...
          const thing = new MessageEmbed()
            ...
            .setDescription(`${emojiaddsong} **Gòi gòi thêm dô hàng chờ ròi**\n[${track.title}](${track.uri}) - \`[${convertTime(track.duration)}]\``)
      ...
      case 'PLAYLIST_LOADED':
        ...
        const thing = new MessageEmbed()
          .setColor(client.embedColor)
          .setTimestamp()
          .setDescription(`${emojiplaylist} **Gòi thêm nguyên đống đó vô hàng chờ ròi **\n${res.tracks.length} mấy bài này nè: [${res.playlist.name}](${search}) - \`[${convertTime(res.playlist.duration)}]\``)
        return message.channel.send({ embeds: [thing] });
      case 'SEARCH_RESULT':
        var track = res.tracks[0];
        player.queue.add(track);
