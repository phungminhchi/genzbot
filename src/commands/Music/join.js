  ...
  execute: async (message, args, client, prefix) => {
    ...
    let player = message.client.manager.get(message.guildId);
        if(player && player.voiceChannel && player.state === "CONNECTED") {
            return await message.channel.send({embeds: [new MessageEmbed().setColor(client.embedColor).setDescription( `Tao đang ở trong <#${player.voiceChannel}> với m nè`)]})
        } else {
    ...
    const emojiJoin = message.client.emoji.join;
     ...
     player = message.client.manager.create({
      ...
      let thing = new MessageEmbed()
        ...
        .setDescription(`${emojiJoin} **Dô phòng đó ròi**\nĐang ở trong đó <#${channel.id}> Chat ở <#${message.channel.id}> á`)
